...
5. **Translate the changes.**
   ...
   - **`border-radius` and `box-shadow`** are ordinary single values. If the source authored per-corner radii (`border-radius: 12px 12px 0 0`), the patch's single value would flatten all four corners - ask rather than flatten.
