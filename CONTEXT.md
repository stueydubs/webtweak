...
- **The server watches the served tree and pushes reloads, so the loop is visible.** Reconcile happens in another window; before this, the user saved and had no signal until they reloaded by hand. webtweak watches its served directory and sends a `source-change` event over SSE (`GET /__webtweak__/events`); a clean page reloads itself, so a reconcile lands in front of the user. Two rules make it safe: a reload **never** runs over unsaved edits (guarded on `dirty`, not on "has edits" - after a Save the batch is on disk and `restore()` re-applies it, and reloading after a save is exactly the case that matters), and webtweak's own artefacts are excluded from the watch or every Save would bounce the page. Watching is per-directory rather than `fs.watch(recursive)`, which only reached Linux in Node 20 while the package supports 18; if watching fails the tool degrades to no live reload, never to a crash. A **reconcile badge** reflects the edits file's own view: `N pending` until Claude flips the batch to `reconciled`.
