...
21. As a site builder, I want the tool to run with no install step and no dependencies, so that I can use it anywhere without dependency management. *(Delivered as Node rather than Python - see the stack note below.)*
...
- **Stack: ~~Python stdlib only~~ -> Node stdlib only.** *(Superseded at `cfe23ff`, before 0.2.0. The tool ships as `webtweak.js` over `node:http`, distributed on npm and run with `npx webtweak`; `engines.node` is `>=18`. Still zero runtime dependencies, which was the decision that actually mattered. Everything below naming Python functions or `unittest` is period text - the shipped equivalents are `injectOverlay` / `applyBatch`, tested from Python via `tests/_wtjs.py`, which drives the real webtweak.js. See CONTEXT.md.)* Originally: single `webtweak` executable over `http.server` / `ThreadingHTTPServer`. interact.js is vendored under `overlay/` (not CDN-loaded) for offline use.
...
- **CLI**: `webtweak <path-to-html> [--root DIR] [--port 8723] [--no-browser] [--install-skill]`; auto-opens the browser. `--root` serves a web root above the page so root-absolute asset URLs resolve; `--install-skill` copies the reconcile skill into `~/.claude/skills/`. Both post-date this PRD and are documented in README + CONTEXT.md.
...
- v2 candidates, in rough priority: structural reordering, then container queries (the natural follow-on from per-breakpoint authoring, and blocked on the same sweep needing a container to resolve against). ~~a font-family dropdown populated from the site's actual `@font-face`/stack declarations~~ - **shipped**, issue 0008, and per-breakpoint authoring was the third and shipped in 0.7.0. This list had gone stale in both directions; the surviving entries are the real ones.
- **Shipped (post-v1): shape creation** - drawing decorative shapes (square/circle/triangle/star/pentagon/hexagon - originally nine, narrowed to six per ADR-0002's amendment note) onto the page, the tool's first element-creation feature. Delivered via a new `op: "create"` Patch and an inline-SVG renderer with free absolute placement; the reconcile skill gained an insert path. The v1 "appearance + layout only, no DOM creation" scope is correspondingly narrowed (created shapes carry `fill`/`stroke`/`stroke-width`/`rx`, unlike the v1 *editing* set which still excludes borders on existing elements). See [ADR-0002](./adr/0002-shape-creation.md).
